...
export default function Quiz() {
  ...
  const handleNext = () => { if (currentQIndex < mockQuestions.length - 1) setCurrentQIndex(prev => prev + 1); };
    ...
    <div className="flex flex-col items-center justify-start w-full py-12 px-4 shadow-inner min-h-full">
      ...
      <div className="glass-card w-full max-w-4xl p-10 md:p-14 flex flex-col min-h-[600px] relative overflow-hidden transition-shadow duration-500 hover:shadow-2xl hover:shadow-indigo-100/50">
        ...
        <div className="flex justify-between items-center mb-8 relative z-10">
          ...
          {/* Timer */}
          <div className="flex items-center gap-2 px-5 py-2.5 rounded-xl font-black text-lg bg-indigo-50 border border-indigo-100 text-indigo-700 shadow-sm">
            <Clock size={20} className={timeLeft < 300 ? "text-red-500 animate-ping" : ""} />
            <span className="tracking-wider">{String(minutes).padStart(2, '0')}:{String(seconds).padStart(2, '0')}</span>
          </div>
        </div>
